...
# ── 세션 초기화 ──────────────────────────────────────────────
for k, v in {"live_results":[], "show_results":False, "search_error":None, "page_num":0}.items():
    if k not in st.session_state:
        st.session_state[k] = v

# ── 헬퍼 함수 ────────────────────────────────────────────────
def load_tracked_db() -> pd.DataFrame:
    try:
        ...
        return df if not df.empty else pd.DataFrame(
            columns=["product_id","site","name","supply_price","delivery_fee",
                     "status","is_tracked","updated_at"])
    except Exception as e:
        st.error(f"DB 오류: {e}")
        return pd.DataFrame()


def apply_margin(df: pd.DataFrame, fee: float, margin: float) -> pd.DataFrame:
    df = df.copy()
...
def do_live_search():
    ...
    pg_size, max_pg = FETCH_PRESETS.get(preset, (50, 2))

    if not DOME_KEY:
        st.session_state["search_error"] = "API 키가 없습니다. Secrets에 DOMEGGOOK_API_KEY를 등록하세요."
    ...
    try:
        ...
        st.session_state["search_error"] = f"API 오류: {e}"

    if not results and not st.session_state["search_error"]:
        st.session_state["search_error"] = "검색 결과가 없습니다. 키워드나 카테고리를 바꿔보세요."
    else:
...
def ai_analyze(tracked_df: pd.DataFrame, fee: float, margin: float,
    ...
    for _, row in tracked_df.iterrows():
        items_summary.append(
            f"- {row['name'][:35]}"
            f" | {row.get('site','')} | 공급가 {int(row['supply_price']):,}원"
            f" | 배송비 {int(row['delivery_fee']):,}원"
            f" | 추천판매가 {int(row['추천 판매가(원)']):,}원"
            f" | 순수익 {int(row['예상 순수익(원)']):,}원"
            f" | 마진율 {row['마진율(%)']:.1f}%"
            f" | 업체등급 {row.get('seller_grade','미확인')}"
        )

    prompt = f"""당신은 20년 경력의 이커머스 소싱 전문가입니다.
아래 관심 상품 목록을 분석하여 소싱 가치를 평가해주세요.

[판매 설정] 플랫폼: {platform_name} (수수료 {int(fee*100)}%) | 마진 전략: {margin_name}
...
2. 핵심 근거 (마진율·업체등급·경쟁력 기준, 2줄 이내)
3. 리스크 (있다면만)

마지막에 전체 포트폴리오 총평 2-3줄.
한국어로 간결하게."""
...
with st.sidebar:
    ...
    fetch_preset = st.selectbox(
        "검색 수량", list(FETCH_PRESETS.keys()), index=1, key="fetch_preset",
        help="많을수록 느림. 대량 수집은 10~20초 소요.")
    ...
    st.markdown(f"""
    <div style="background:#0f3460;border-radius:10px;padding:.8rem 1rem;margin-top:.5rem">
...
with tab_search:
    ...
    if not st.session_state["show_results"]:
        st.markdown("""<div style="text-align:center;padding:3rem 0;color:#94a3b8">
            ...
            <div style="font-size:.85rem;margin-top:.3rem">도매매·도매꾹 실시간 데이터 | 최대 500개 수집</div>
        </div>""", unsafe_allow_html=True)
    else:
        ...
        if raw_df.empty:
            ...
            st.markdown(f"<div style='color:#64748b;font-size:.85rem;margin-bottom:.5rem'>📊 {badge} | 총 {len(raw_df):,}개</div>",
                        unsafe_allow_html=True)

            # 필터 바
            fc1, fc2, fc3, fc4, fc5 = st.columns([2, 1, 1, 1.2, 1.2])
            with fc1:
                kw_filter = st.text_input("결과 내 재검색", "", placeholder="상품명 필터...",
                                          label_visibility="collapsed")
            with fc2:
                site_filter = st.selectbox("소싱처", ["전체"]+list(site_counts.keys()),
                                           label_visibility="collapsed")
            with fc3:
            ...
            if "seller_grade" not in df.columns:
                df["seller_grade"] = "조회중"
            ...
            st.caption("💡 업체등급은 ⭐ 관심 등록 시 상세 API로 자동 갱신됩니다.")
            ...
            # ★ 스크롤 앵커 — 체크 후 이 위치로 복귀
            st.markdown('<div id="search-anchor"></div>', unsafe_allow_html=True)
                ...
                if edited.iloc[i]["⭐ 관심"] != disp.iloc[i]["⭐ 관심"]:
                    ...
                    if is_track and client and prod.get("site") in ["도매매","도매꾹"]:
                        ...
                        if detail:
                            ...
                            prod["seller_grade"]  = detail.get("seller_grade", prod.get("seller_grade",""))
            ...
            with pn2:
                st.markdown(f"<div style='text-align:center;color:#64748b;font-size:.85rem;padding-top:.5rem'>"
                            f"{page_num+1} / {total_pg} 페이지 | 총 {len(df):,}개</div>",
                            unsafe_allow_html=True)
            with pn3:
...
with tab_monitor:
    ...
    if tracked_df.empty:
        st.markdown("""<div style="text-align:center;padding:3rem 0;color:#94a3b8">
            ...
            <div style="font-size:.85rem;margin-top:.3rem">검색 탭에서 ⭐를 체크하면 여기에 저장됩니다</div>
        </div>""", unsafe_allow_html=True)
    else:
        total_m = len(tracked_df)
        oos_m   = len(tracked_df[tracked_df["status"] != "Y"])
        tracked_df = apply_margin(tracked_df, fee, margin)
        avg_m = int(tracked_df[tracked_df["status"]=="Y"]["예상 순수익(원)"].mean()) \
                if total_m - oos_m > 0 else 0
        ...
        with mf1:
            m_filter = st.text_input("상품명 검색", "", placeholder="관심 상품 내 검색...",
                                     label_visibility="collapsed")
        with mf2:
        ...
        if "seller_grade" not in mdf.columns:
            mdf["seller_grade"] = "- 미확인"
        if m_filter:
        ...
        # ★ 관심상품 테이블 컬럼 명시적으로 선택 및 한글 변환
        keep = ["❌ 해제","site","product_id","name","supply_price","delivery_fee",
        ...
        edited_m = st.data_editor(
            ...
            column_config={
                ...
                "등록시간":        st.column_config.TextColumn("등록시간",
                    help="소싱레이더 DB에 저장된 시간 (업체 가격 변경 시간 아님)"),
            },
        ...
        if not GEMINI_KEY:
            ...
            if st.button("✨ AI 소싱 분석 시작", type="primary", use_container_width=True):
                with st.spinner("AI가 분석 중입니다... (10~20초 소요)"):
                    result = ai_analyze(tracked_df, fee, margin, platform_name, margin_name)
                if result:
                    st.markdown("**📊 AI 분석 결과**")
                    st.markdown(result)
                    if st.button("📱 분석 결과를 텔레그램으로 전송"):
                        send_telegram_message(f"🤖 AI 소싱 분석 결과\n\n{result[:1000]}")
                        st.success("전송 완료!")
                else:
                    st.error("AI 분석 실패. API 키를 확인하세요.")
...
""", language="toml")
        ...
        if st.button("📤 테스트 발송", type="primary", use_container_width=True):
            ...
            st.success("✅ 발송 성공!") if ok else st.error("❌ 실패")
...
with tab_guide:
    ...
    with st.expander("🏪 소싱 업체", expanded=True):
        st.markdown("""
| 업체 | 방식 | 특징 |
|------|------|------|
...
> 💡 검색 결과의 "업체등급"은 ⭐ 관심 등록 시 실제 값으로 자동 갱신됩니다.
        """)

    with st.expander("📊 검색 수량 기준"):
        st.markdown("""
| 옵션 | 총 상품 수 | 소요 시간 |
|------|-----------|----------|
| 빠른 탐색 | 50개 | 1~2초 |
