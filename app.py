...
def do_track(prod, is_track):
    """관심 등록/해제 — 등록 시 상세 API로 등급/배송비 갱신"""
...
def render_card_grid(df, tracked_ids, id_prefix="live", on_toggle=None):
    """공통 카드 그리드 렌더링"""
    per_row=3
        ...
        for ci,(_,row) in enumerate(df.iloc[i:i+per_row].iterrows()):
            with cols[ci]:
                ...
                if img and img.startswith("http"):
                    ...
                    st.markdown("<div style='height:100px;background:#f1f5f9;border-radius:8px;"
...
<div style="font-size:.72rem;color:#64748b">
    마진 <b>{margin_p:.1f}%</b> | 등급 <span style="color:{gc};font-weight:600">{grade or '조회중'}</span> | {row.get('상태','')}
</div>""",unsafe_allow_html=True)

                label="⭐ 등록됨" if is_t else "☆ 관심 등록"
                if st.button(label,key=f"{id_prefix}_card_{row['product_id']}",use_container_width=True):
                    on_toggle(row.to_dict(), 0 if is_t else 1)
                    st.rerun()

def compact_ai_analyze(items_df, fee, margin, platform_name, margin_name):
    """Gemini 소싱 분석 — 핵심 축약형 (텔레그램 전송 최적화)"""
    if not GEMINI_KEY:
    ...
    for _,r in items_df.iterrows():
        ...
        rows.append(
            f"• {r['name'][:30]} ({r.get('site','')})\n"
            f"  공급{supply:,}+배송{delivery:,}={supply+delivery:,}원 | "
            f"판매가{sale:,}원 | 순수익{profit:,}원 | 마진{mp:.1f}% | 등급:{grade}"
        )

    prompt=f"""당신은 20년 경력 이커머스 소싱 전문가입니다.
마케팅·세일즈 관점에서 핵심만 간결하게 분석해주세요.

[설정] {platform_name} 수수료{int(fee*100)}% | {margin_name}({int(margin*100)}%)
...
📊 개별 판정 (모든 상품)
[상품명] ✅/👍/⚠️/❌ | 마진경쟁력: 상/중/하 | 리스크: 한 줄 | 월50건수익: {'{금액}'}원
...
def send_tg_long(text: str, prefix: str="🤖 AI 소싱 분석"):
    """긴 텍스트를 3800자씩 분할해서 텔레그램 전송"""
    chunk_size=3800
    ...
    for i,chunk in enumerate(chunks):
        header=f"{prefix}"
        if total>1: header+=f" ({i+1}/{total})"
        msg=f"{header}\n\n{chunk}"
        ok=send_telegram_message(msg)
        if not ok: success=False
        if i<len(chunks)-1: import time; time.sleep(0.5)
    return success
...
with st.sidebar:
    ...
    if OC_AVAILABLE:
        st.caption(f"온채널: {'✅ 연동됨' if (OC_ID and OC_PW) else '⚠️ ID/PW 미설정'}")
    ...
    ok_t=total_t-oos_t
    avg_profit_t=int(apply_margin(t_df,fee,margin)["예상 순수익(원)"].mean()) if not t_df.empty else 0
    st.markdown("#### 📊 관심 상품")
    c1,c2=st.columns(2); c1.metric("전체",total_t); c2.metric("🟢 정상",ok_t)
    st.markdown("---")
    st.markdown("#### 📱 텔레그램")
    ...
    if st.button("🔔 테스트 발송",use_container_width=True):
        ok=send_telegram_message(f"✅ 소싱레이더!\n관심 상품: {total_t}개 | 평균수익: {avg_profit_t:,}원")
        st.success("발송!") if ok else st.error("실패")
    st.markdown("---")
    st.markdown(f"#### 🤖 AI (Gemini 무료)\n{'✅ 사용 가능' if GEMINI_KEY else '⚠️ 미설정'}")
    st.markdown("---")
    st.caption("소싱레이더 v5.1")
...
with tab_search:
    ...
    with b3:
        v_sel=st.selectbox("보기",["📋 표","🖼 카드"],key="live_view_sel",label_visibility="collapsed")
    ...
    if not st.session_state["show_results"]:
        ...
        if raw_df.empty:
            ...
            st.caption("💡 업체등급 '조회중' → ⭐ 관심 등록 시 실제 등급으로 자동 갱신")
            ...
            if st.session_state["live_view"]=="card":
                render_card_grid(page_df, tracked_ids, id_prefix="live",
...
with tab_monitor:
    ...
    if tracked_df.empty:
        ...
        tracked_calc=apply_margin(tracked_df,fee,margin)

        # ★ KPI — 품절 대신 평균마진율·월예상수익 표시
        ok_m=len(tracked_df[tracked_df["status"]=="Y"])
        avg_margin=round(tracked_calc["마진율(%)"].mean(),1) if not tracked_calc.empty else 0
        avg_profit=int(tracked_calc[tracked_calc["status"]=="Y"]["예상 순수익(원)"].mean()) if ok_m>0 else 0
        monthly_est=avg_profit*100  # 월 100건 기준 예상
        ...
        k3.markdown(f'<div class="kpi-card"><div class="kpi-label">평균 마진율</div><div class="kpi-value" style="color:#7c3aed">{avg_margin:.1f}%</div><div class="kpi-sub">목표마진 기준</div></div>',unsafe_allow_html=True)
        k4.markdown(f'<div class="kpi-card"><div class="kpi-label">월 예상수익 (100건×평균)</div><div class="kpi-value" style="color:#2563eb">{monthly_est:,}</div><div class="kpi-sub">원</div></div>',unsafe_allow_html=True)
        st.markdown("<br>",unsafe_allow_html=True)

        # 보기 방식 + 필터
        mv1,mv2,mv3=st.columns([2,1,1])
        with mv1: m_filter=st.text_input("상품명 검색","",placeholder="관심 상품 내 검색...",label_visibility="collapsed")
        with mv2: m_status=st.selectbox("상태",["전체","정상만","품절만"],label_visibility="collapsed")
        with mv3:
            mv_sel=st.selectbox("보기",["📋 표","🖼 카드"],key="mon_view_sel",label_visibility="collapsed")
            st.session_state["mon_view"]="card" if "카드" in mv_sel else "table"

        mdf=tracked_calc.copy()
        ...
        if st.session_state["mon_view"]=="card":
            render_card_grid(mdf, tracked_ids_m, id_prefix="mon",
                             on_toggle=lambda p,t: (upsert_tracked_product(p,t)))
        ...
        # ── 표 보기 ──────────────────────────────────────────
            ...
            if "updated_at" in mdf.columns:
                mdf["등록시간"]=mdf["updated_at"].apply(fmt_dt)

            keep=["🤖 AI선택","❌ 해제","site","product_id","name","supply_price","delivery_fee",
                  "상태","추천 판매가(원)","예상 순수익(원)","마진율(%)","seller_grade","등록시간"]
            ...
            edited_m=st.data_editor(mdf_disp,use_container_width=True,hide_index=True,key="tracked_editor",
                column_config={
                    "🤖 AI선택":st.column_config.CheckboxColumn("🤖 AI선택",width="small",help="AI 분석 포함 상품 선택"),
                    ...
                    "등록시간":st.column_config.TextColumn("등록시간",help="소싱레이더 DB 저장 시간 (업체 변경 시간 아님)"),
                },
        ...
        if st.button("📱 관심 상품 현황을 텔레그램으로 전송",use_container_width=True):
            lines=[f"📊 관심 상품 현황\n전체 {total_m}개 | 정상 {ok_m}개 | 평균마진 {avg_margin:.1f}%\n"]
            for _,row in tracked_calc.iterrows():
                icon="🟢" if row["status"]=="Y" else "❌"
                lines.append(f"{icon} {row['name'][:22]} — 판매가{int(row['추천 판매가(원)']):,}원|수익{int(row['예상 순수익(원)']):,}원")
            ok=send_tg_long("\n".join(lines),"📊 관심 상품 현황")
            st.success("✅ 전송 완료!") if ok else st.error("❌ 전송 실패")

        # ── AI 소싱 분석 ──────────────────────────────────────
        st.markdown("---")
        st.markdown("#### 🤖 AI 소싱 분석 (핵심 축약형)")

        if not GEMINI_KEY:
            st.info("💡 [aistudio.google.com](https://aistudio.google.com)에서 무료 키 발급 후 Secrets에 `GEMINI_API_KEY` 등록")
        else:
            # 표 보기일 때만 AI 선택 가능
            if st.session_state["mon_view"]=="table" and "edited_m" in dir():
                selected_rows=edited_m[edited_m["🤖 AI선택"]==True]
                sel_count=len(selected_rows)
            else:
                selected_rows=pd.DataFrame(); sel_count=0

            if sel_count>0:
                st.success(f"✅ {sel_count}개 선택됨")
            else:
                st.info("💡 표 보기에서 **🤖 AI선택** 체크 후 분석, 또는 전체 분석 버튼 사용")
            ...
            if all_btn:
                with st.spinner(f"전체 {total_m}개 분석 중..."):
                    result=compact_ai_analyze(tracked_calc,fee,margin,platform_name,margin_name)
                st.session_state["ai_result"]=result

            if ai_btn and sel_count>0:
                sel_names=selected_rows["상품명"].tolist()
                sel_df=tracked_calc[tracked_calc["name"].isin(sel_names)].copy()
                with st.spinner(f"{sel_count}개 분석 중..."):
                    result=compact_ai_analyze(sel_df,fee,margin,platform_name,margin_name)
                st.session_state["ai_result"]=result
            ...
            if st.session_state.get("ai_result"):
                ...
                with col_t1:
                    if st.button("📱 분석 결과를 텔레그램으로 전송",use_container_width=True,key="ai_tg_btn"):
                        ok=send_tg_long(result_text,"🤖 AI 소싱 분석")
                        st.success("✅ 전송 완료!") if ok else st.error("❌ 전송 실패")
                with col_t2:
...
with tab_guide:
    st.markdown("### 📖 가이드")
    with st.expander("🖼 카드/표 보기 전환"):
        st.markdown("검색 결과·관심 상품 모두 **📋 표** / **🖼 카드** 전환 가능. 카드는 이미지+핵심 정보 한눈에.")
...
텔레그램 전송 시 자동으로 분할 발송합니다.
        """)
    with st.expander("💡 마진 공식"):
        st.markdown("""
```
추천 판매가 = (공급가+배송비) ÷ (1-수수료율-마진율)
