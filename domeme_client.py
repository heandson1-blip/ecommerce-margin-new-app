...
class DomeameClient:
    ...
    def fetch_product_list(
        ...
        page_size: int = 50,
        max_pages: int = 1,        # ★ 페이지 수 (1페이지=50개, 4페이지=200개)
    ) -> list[dict]:
        """
        도매매/도매꾹 상품 목록 조회.
        page_size: 페이지당 상품 수 (10~100 권장)
        max_pages: 최대 호출 페이지 수 (많을수록 느림)
        총 최대 결과 = page_size × max_pages
        """
        ...
        for pg in range(1, max_pages + 1):
            ...
            try:
                ...
                if resp.status_code != 200:
                    ...
                    break
                page_data = self._parse(resp.text, site_name)
                if not page_data:          # 더 이상 결과 없음 → 중단
                    break
                all_results.extend(page_data)
                print(f"[{site_name}] pg={pg} → {len(page_data)}개 (누적 {len(all_results)}개)")
                if len(page_data) < page_size:  # 마지막 페이지
                    break
                if pg < max_pages:
    ...
    def fetch_item_detail(self, product_id: str) -> dict | None:
        ...
        try:
            ...
            return {"status": status, "supply_price": int(raw) if raw.isdigit() else 0}
        except Exception as e:
            print(f"[상품조회 오류] {product_id}: {e}")
            return None
    ...
    def _parse(self, raw: str, site_name: str) -> list[dict]:
        ...
        try:
            ...
            for item in items:
                ...
                delivery_fee = (
                    int(raw_delivery)
                    if raw_delivery.isdigit() and int(raw_delivery) > 0
                    else 3000
                )
                results.append({
                    "site":         site_name,
